...
from . import time_features as ft
from . import wavelet_features as fw
from . import phase_features as fp
from . import frequency_features as ff

from ..utils.utils_data import split_into_epochs, \
...
def generate_time_features(crops, sfreq, agg_func, axis=-1):
    ...
    fisher_info = ft.compute_fisher_information(epochs=crops, axis=axis, Tau=Tau, DE=DE)
    fractal_dim = ft.compute_fractal_dimension(epochs=crops, axis=axis)
    higuchi_fractal_dim = ft.compute_higuchi_fractal_dimension(epochs=crops, axis=axis, Kmax=Kmax)
    [activity, mobility, complexity] = ft.compute_hjorth_parameters(epochs=crops, axis=axis)
    hurst_exp = ft.compute_hurst_exponent(epochs=crops, axis=axis)
    ...
    petrosian_fractal_dim = ft.compute_petrosian_fractal_dimension(epochs=crops, axis=axis)
    skew = ft.skewness(epochs=crops, axis=axis)
    svd_entropy_ = ft.compute_svd_entropy(epochs=crops, axis=axis, Tau=Tau, DE=DE, W=W)
    ...
    time_features = np.hstack((
        ...
        complexity, mobility, hurst_exp, kurt, linelength, max_,
